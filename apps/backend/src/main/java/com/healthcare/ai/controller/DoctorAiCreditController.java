...
@RestController
@RequestMapping("/api/v1/doctor/ai-credits")
@PreAuthorize("hasRole('DOCTOR')")
public class DoctorAiCreditController {

    private final AiCreditService aiCreditService;
    private final UserRepository userRepository;

    public DoctorAiCreditController(AiCreditService aiCreditService, UserRepository userRepository) {
    ...
    public ResponseEntity<Map<String, Object>> getStatus(@AuthenticationPrincipal UserDetails principal) {
        ...
        int credits = aiCreditService.getDoctorCredits(user.getId());
