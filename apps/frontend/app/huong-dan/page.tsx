  ...
  return (
    <PublicPageShell branches={branches?.content ?? []}>
      <div className="resource-page section-inner">
        <div className="resource-breadcrumb"><Link href="/">Trang chủ</Link><span>/</span><span>Hướng dẫn</span></div>
        <header className="resource-page__header">
          <p className="section-note">Hướng dẫn đặt khám</p>
          <h1>Một lộ trình khám rõ ràng hơn</h1>
          <p>Tìm hiểu các bước chọn chuyên khoa, bác sĩ, cơ sở và khung giờ để chuẩn bị thuận tiện hơn cho cuộc hẹn.</p>
        </header>
        ...
        <section className="resource-panel resource-panel--wide">
          <div className="section-heading">
            <div><p className="section-note">Hành trình đặt khám</p><h2>Từ nhu cầu tới cuộc hẹn</h2></div>
            <PublicAiButton className="outline-button">Hỗ trợ chọn chuyên khoa</PublicAiButton>
          </div>
          <div className="resource-steps resource-steps--grid">{JOURNEY.map(([number, title, description]) => <div className="resource-step-card" key={number}><span>{number}</span><strong>{title}</strong><p>{description}</p></div>)}</div>
        </section>
