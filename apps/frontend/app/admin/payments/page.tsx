...
export default function AdminPaymentsPage() {
  ...
  const review = async (item: BankTransferPayment, decision: "VERIFY" | "REJECT") => {
    ...
    if (decision === "REJECT") {
      ...
      if (!reason) return;
    } else if (!window.confirm(`Xác nhận đã nhận ${money(item.amount)} cho lịch ${item.bookingCode}?`)) return;
  ...
  return (
    <div>
      ...
      {!loading && items.length > 0 ? (
        <div className="mt-4 max-w-full overflow-x-auto rounded-lg border border-slate-200 bg-white" role="region" tabIndex={0}>
          <table className="w-full min-w-[1050px] text-left text-sm">
            ...
            <tbody>{items.map((item) => <tr className="border-b border-slate-100 align-top last:border-0" key={item.id}>
              ...
              <td className="px-4 py-4">{item.status === "PENDING_VERIFICATION" ? <div className="flex gap-2"><button className="rounded-lg bg-teal-700 px-3 py-2 text-xs font-bold text-white disabled:opacity-50" disabled={reviewing === item.id} onClick={() => void review(item, "VERIFY")} type="button">Xác nhận đã nhận</button><button className="rounded-lg border border-red-300 px-3 py-2 text-xs font-bold text-red-700 disabled:opacity-50" disabled={reviewing === item.id} onClick={() => void review(item, "REJECT")} type="button">Từ chối</button></div> : item.status === "REFUND_PENDING" ? <button className="rounded-lg bg-amber-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50" disabled={reviewing === item.id} onClick={() => void refund(item)} type="button">Xác nhận đã hoàn</button> : item.refundReference ? <span className="font-mono text-xs">{item.refundReference}</span> : "—"}</td>
            </tr>)}</tbody>
          </table>
        </div>
      ) : null}
