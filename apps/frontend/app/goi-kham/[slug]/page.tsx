    ...
    <div className="min-h-screen flex flex-col bg-sand-100 text-ink font-sans">
      ...
      <main className="flex-1 py-12 px-4 sm:px-6 max-w-6xl mx-auto w-full space-y-12">
        ...
        <div className="text-xs text-ink-muted flex items-center gap-2">
          ...
          <span>/</span>
          <Link href="/#packages" className="hover:text-brand-700">Gói khám</Link>
          <span>/</span>
          <span className="text-brand-900 font-semibold">{pkg.name}</span>
        </div>

        {/* Hero Package Overview */}
        <div className="bg-gradient-to-r from-brand-950 via-brand-900 to-brand-800 text-white p-8 sm:p-12 rounded-3xl shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          ...
          <div className="bg-white/10 border border-white/20 p-6 rounded-2xl backdrop-blur-md text-right min-w-[240px]">
            <span className="text-xs text-brand-300 block">Chi phí trọn gói niêm yết:</span>
        ...
        <section className="bg-white p-8 rounded-3xl border border-mint-100 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs sm:text-sm">
            <div className="p-4 bg-sand-100 rounded-2xl border border-mint-100 space-y-2">
              <span className="font-bold text-brand-900 block text-sm">1. Khám Lâm Sàng Chuyên Khoa</span>
              <ul className="space-y-1 text-ink-muted list-disc list-inside text-xs">
            ...
            <div className="p-4 bg-sand-100 rounded-2xl border border-mint-100 space-y-2">
              <span className="font-bold text-brand-900 block text-sm">2. Xét Nghiệm Huyết Học & Sinh Hóa</span>
              <ul className="space-y-1 text-ink-muted list-disc list-inside text-xs">
            ...
            <div className="p-4 bg-sand-100 rounded-2xl border border-mint-100 space-y-2">
              <span className="font-bold text-brand-900 block text-sm">3. Chẩn Đoán Hình Ảnh & Thăm Dò Chức Năng</span>
              <ul className="space-y-1 text-ink-muted list-disc list-inside text-xs">
            ...
            <div className="p-4 bg-sand-100 rounded-2xl border border-mint-100 space-y-2">
              <span className="font-bold text-brand-900 block text-sm">4. Tầm Soát Dấu Ấn Ung Thư Sớm</span>
              <ul className="space-y-1 text-ink-muted list-disc list-inside text-xs">
        ...
        <section className="p-6 bg-brand-50 border border-brand-200 rounded-3xl text-xs space-y-2">
          <span className="font-bold text-brand-950 text-sm block">📌 Chuẩn bị trước khi đi khám gói sức khỏe:</span>
          <p className="text-brand-900">1. Nhịn ăn tối thiểu 6 đến 8 tiếng trước khi lấy máu (có thể uống nước lọc).</p>
          <p className="text-brand-900">2. Không uống rượu bia, nước ngọt hoặc dùng chất kích thích trước ngày khám 24 giờ.</p>
          <p className="text-brand-900">3. Phụ nữ không nên khám phụ khoa trong kỳ kinh nguyệt.</p>
        </section>
      </main>
