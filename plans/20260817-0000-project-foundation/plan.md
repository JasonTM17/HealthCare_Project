...
blockedBy: []
blocks: []
evidence_state: implementation-in-progress
---
...
- In scope: Git/provenance plan; root `README.md`; root `.gitignore` with careful secret and generated-file exclusions; `.env.example`; foundation layout for `apps/backend`, `apps/frontend`, `apps/ai-service`, `docs/adr`, `docs/architecture`, `infrastructure`; baseline commands, tests, health checks, Docker/dev infrastructure; frontend baseline using `ak:frontend-design` and, when coding, `ak:frontend-development`; Stitch/MCP preflight and static concept generation if capability is available; backend identity/authentication and RBAC implementation, including Flyway migration, registration/login/refresh/logout endpoints, JWT, and regression tests; public and administrator-managed hospital catalog APIs for specialties, doctors, branches, services, packages, articles, and FAQs.
- Out of scope: copying Hoan My logo, brand name, colors, photos, doctors, medical copy, addresses, phone numbers, or proprietary assets; appointment, clinical, or AI RAG implementation; frontend authentication flows; deploy; push/commit; credentialed provider calls without approval/capability; automatic `/goal` start; real secret values in artifacts.
...
- Objective: validate and complete the already-present backend identity/authentication slice without adding hospital-domain capabilities.
- Deliverables: Flyway identity/RBAC migration; UUID user/role/permission/refresh-token persistence; BCrypt password hashing; JWT access and refresh tokens; registration, login, refresh, logout, and current-user API boundaries; authorization configuration; ADR and schema documentation; regression tests.
- Acceptance signals: backend tests cover registration, duplicate-email rejection, invalid login, protected access, logout, refresh-token rotation/reuse rejection, and migrations; API responses never expose password hashes; configuration has no duplicate YAML keys or real secrets; authenticated API contract and remaining transport-security decision are documented.

### Phase 6: Public Hospital Catalog And Administration

- Objective: validate the present hospital catalog implementation without adding appointments, clinical records, or copied third-party content.
- Deliverables: Flyway migration for specialties, doctors, branches, services, packages, articles, FAQs, and doctor relationship tables; public active/published catalog APIs; administrator CRUD APIs protected by `ADMIN` role; controller/integration regression tests.
- Acceptance signals: schema validates against PostgreSQL; public endpoints omit inactive/unpublished entities; administrator writes reject unauthenticated/non-admin callers; validation, duplicate slugs, missing resources, and role denial have regression tests; all visible content remains original/fictitious.
...
| 5 | Backend identity/auth/RBAC | Migration, secure endpoints, tests, ADR/schema docs | User authorized extending this plan on 2026-08-17 after auth/JWT/RBAC code was found in the workspace |
| 6 | Hospital catalog/admin APIs | V2 migration, public catalog, admin CRUD, authorization tests | User authorized extending this plan on 2026-08-17 after hospital-domain code was found in the workspace |
